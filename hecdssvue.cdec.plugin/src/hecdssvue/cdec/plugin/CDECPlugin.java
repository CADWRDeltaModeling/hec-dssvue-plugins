...
public class CDECPlugin {
	...
	public static String chooseFileToOpenOrSave(
			final ListSelection listSelection, String dialogTitle,
		...
		chooser.setFileFilter(new rma.util.RMAFilenameFilter(extensionName,
				extension));
		if (open) {
	...
	protected void process(final ListSelection listSelection) {
		...
		//
		JFrame fr = new JFrame("CDEC Downloader");
		//
		JPanel controlPanel = new JPanel();
		final JToggleButton showSelectedSensors = new JToggleButton(
				"Show Selected Only");
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							for (CDECSensor sensor : stationTableModel
									.getSelected()) {
								progress.setMessage("Starting download for : "
										+ sensor.getStationId()
										+ "/"
										+ sensor.getId()
										+ "/"
										+ sensor.getType()
										+ (sensor.getSubType().length() > 0 ? ", "
												: "") + sensor.getSubType());
								String timeWindow = getTimeWindowStringFromListSelection();
								TimeSeriesContainer tsc = new CDECStationWebService()
										.retrieveSensorData(sensor, timeWindow, progress);
								progress.setMessage("Saving " + tsc.fullName
										+ " to dss file");
		...
		Action saveSelectionAction = new SaveSelectionAction(
				"Save Selected...", null, "Saves selection to file.",
				new Integer(KeyEvent.CTRL_DOWN_MASK | KeyEvent.VK_S));
		Action loadSelectionAction = new LoadSelectionAction(
				"Load Selected...", null, "Loads selection from file",
		...
		Action recacheAction = new AbstractAction("Recache Station Sensor List") {
		...
		JOptionPane.showMessageDialog(
				...
				"Error: "
						...
						+ stringWriter.getBuffer()
								.substring(
										0,
										Math.max(30, stringWriter.getBuffer()
	...
	protected void loadSelected(String loadFromFile) throws Exception {
		...
		Gson gson = new Gson();
		List<CDECSensor> selectedSensors = gson.fromJson(new FileReader(
				loadFromFile), new TypeToken<List<CDECSensor>>() {
		}.getType());
		stationTableModel.setSelected(selectedSensors);
		stationTableModel.fireTableChanged(new TableModelEvent(
				stationTableModel));
	}
	...
	public class SaveSelectionAction extends AbstractAction {
		public SaveSelectionAction(String text, ImageIcon icon, String desc,
				Integer mnemonic) {
			super(text, icon);
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			String saveToFile = chooseFileToOpenOrSave(listSelection,
					"Save selection to file", "sel", "*.sel", false);
			try {
	...
	public class LoadSelectionAction extends AbstractAction {
		...
		public LoadSelectionAction(String text, ImageIcon icon, String desc,
				Integer mnemonic) {
			super(text, icon);
			putValue(SHORT_DESCRIPTION, desc);
			putValue(MNEMONIC_KEY, mnemonic);
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			String loadFromFile = chooseFileToOpenOrSave(listSelection,
					"Load selection from file", "sel", "*.sel", true);
			try {
